...
class TestProducts(TestCase):
    def setUp(self):
        ...
        self.category = Category.objects.create(name='Toys', slug='тойз')
        ...
        self.product2 = Product.objects.create(
            name='Футбольный мяч',
            description='Мяч для футбола',
            category = self.category,
            price = 500
        )
        self.product3 = Product.objects.create(
            name='Плюшевый медведь',
            description='Большой медведь',
            category = self.category,
            price = 1500
        )
        self.product_payload = {
            ...
            'category': self.category.id, # т.к. на slug primary key не стоит
            'price': 2000
        }
